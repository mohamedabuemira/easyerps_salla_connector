...
class ProductCategory(models.Model):
    ...
    def x_2_odoo(self, data, mode='create'):
        ...
        if (data.get('parent_id')):
            parent_id = self.search([('salla_id', '=', data.get('parent_id'))], limit=1)
            if parent_id:
                result.update(parent_id=parent_id.id)
    ...
    def odoo_2x_create(self):
        ...
        if company_id:
            response = company_id.odoo_2_x_crud('POST', 'categories', payload_json=record)
    ...
    def odoo_2x_read_all(self):
        ...
        if company:
            company_id = self.env['res.company'].browse(company)
        if company_id:
            response = company_id.odoo_2_x_crud('GET', 'categories', payload=None)
        if response and response.get('data'):
    ...
    def odoo_2x_read(self):
        ...
        if company_id and self.salla_id:
            response = company_id.odoo_2_x_crud('GET', 'categories/' + str(self.salla_id), payload=None)
        if response and response.get('data'):
            self.x_2_odoo(response.get('data'))

    def odoo_2x_update(self, endpoint=None):
        if not endpoint:
            endpoint = 'categories/' + str(self.salla_id)
        record = self.odoo_2_x()
        company = self.env.context.get('company_id', False)
        company_id = self.env.company
        response = None
    ...
    def odoo_2x_delete(self):
        ...
        if company:
            company_id = self.env['res.company'].browse(company)
        if company_id and self.salla_id:
            response = company_id.odoo_2_x_crud('DELETE', 'categories/' + str(self.salla_id), payload=None)
        return response
