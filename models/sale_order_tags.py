...
class OrderTags(models.Model):
    ...
    def odoo_2x_create(self):
        ...
        if company_id:
            response = company_id.odoo_2_x_crud('POST', 'orders/tags', payload_json=record)
    ...
    def odoo_2x_read_all(self):
        ...
        if company:
            company_id = self.env['res.company'].browse(company)
        if company_id:
            response = company_id.odoo_2_x_crud('GET', 'orders/tags', payload=None)
        if response and response.get('data'):
    ...
    def odoo_2x_read(self):
        ...
        if company_id and self.salla_id:
            response = company_id.odoo_2_x_crud('GET', 'orders/tags/' + str(self.salla_id), payload=None)
        if response and response.get('data'):
    ...
    def odoo_2x_update(self,endpoint=None):
        return
        if not endpoint:
            endpoint='orders/tags/' + str(self.salla_id)
        record = self.odoo_2_x()
        company = self.env.context.get('company_id', False)
        company_id = self.env.company
        response = None
    ...
    def odoo_2x_delete(self):
        ...
        if company_id and self.salla_id:
            for rec in self:
                response = company_id.odoo_2_x_crud('DELETE', 'orders/tags/' + str(rec.salla_id), payload=None)
        return response
