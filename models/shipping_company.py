...
class ShippingCompany(models.Model):
    ...
    activation_type = fields.Selection([('api', 'API'), ('manual', 'Manual')],default="manual")
    slug = fields.Char()
    ...
    def x_2_odoo(self, data, mode='create'):
        ...
        if data.get('activation_type') == "Manual":
            result.update(activation_type='manual')
        elif data.get('activation_type') == "Salla":
            result.update(activation_type='api')
    ...
    def odoo_2x_create(self):
        ...
        if company_id:
            response = company_id.odoo_2_x_crud('POST', 'shipping/companies', payload_json=record)
    ...
    def odoo_2x_read_all(self):
        ...
        if company:
            company_id = self.env['res.company'].browse(company)
        if company_id:
            response = company_id.odoo_2_x_crud('GET', 'shipping/companies', payload=None)
        if response and response.get('data'):
    ...
    def odoo_2x_read(self):
        ...
        if company:
            company_id = self.env['res.company'].browse(company)
        if company_id :
            response = company_id.odoo_2_x_crud('GET', 'shipping/companies/' + str(self.salla_id), payload=None)
        if response and response.get('data'):
            self.x_2_odoo(response.get('data'))

    def odoo_2x_update(self, endpoint=None):
        if not endpoint:
            endpoint = 'shipping/companies/' + str(self.salla_id)
        record = self.odoo_2_x()
        company = self.env.context.get('company_id', False)
        company_id = self.env.company
        response = None
    ...
    def odoo_2x_delete(self):
        ...
        if company_id and self.salla_id:
            for rec in self:
                response = company_id.odoo_2_x_crud('DELETE', 'shipping/companies/' + str(rec.salla_id), payload=None)
        return response
